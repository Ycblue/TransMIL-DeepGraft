...
class Visualize():
    ...
    def __init__(self, model, checkpoint_path, project, task):
        super().__init__()
    ...
    def _load_model_from_checkpoint(self, checkpoint):
        ...
        if self.model_name == 'TransMIL':
            ...
            # mil_model = TransMIL(n_classes=self.n_classes, in_features=self.in_features)
            mil_model = CLAM_MB(n_classes=self.n_classes)
            clam_path = '/homeStor1/ylan/workspace/HIA/logs/DeepGraft_Lancet_co2/clam_mb/DEEPGRAFT_CLAMMB_CROSSVAL_norm_rej_rest_1/RESULTS/bestModelFold_1'
            temp_state_dict = torch.load(clam_path)
            new_state_dict = OrderedDict()
            for k, v in temp_state_dict.items():
                name = k
                new_state_dict[name] = v
            mil_model.load_state_dict(new_state_dict)
    ...
    def run(self, target_label):
        ...
        for i in range(repetition):
            ...
            tracker_path = f'/homeStor1/ylan/workspace/TransMIL-DeepGraft/co2log/gradcam'
